...
def find_correct_data_from_soup(soup: BeautifulSoup, item: str) -> BeautifulSoup:
    ...
    print(search_product.prettify())
    
    # check for None
    if not search_product:
        print('No product found (no search results) THIS IS AN ISSUE')
        return None
...
for item in item_csv:
    ...
    if die_count > 10:
        raise Exception('Possible that there may be connection issues. Please check if your connection to the website is stable.')
...
# print(df)
df.to_excel('datart-sk.xlsx', index=False)
